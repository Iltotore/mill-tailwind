...
object Counter extends TyrianIOApp[TodoMsg, TodoList]:
  ...
  def todoEntry(entry: TodoEntry, id: Int): Html[TodoMsg] =
    ...
    div(
      tabindex := 0,
      cls := s"collapse collapse-arrow bg-base-100 $borderColor border"
    )(
      input(tpe := "checkbox"),
      div(
        cls := "collapse-title font-semibold flex flex-row items-center justify-between *:z-2"
      )(
        input(
          tpe := "text",
          value := entry.title,
  ...
  val addBox: Html[TodoMsg] =
    div(
      ...
      input(
        ...
        onKeyUp(event =>
          if event.key == "Enter" then TodoMsg.AddEntry
          else TodoMsg.NoOp
        )
      ),
